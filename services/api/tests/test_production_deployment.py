...
from services.video_export_service import _find_preferred_font  # noqa: E402
...
class ProductionDeploymentTests(unittest.TestCase):
    def test_configured_font_has_priority(self) -> None:
        with tempfile.NamedTemporaryFile(suffix=".ttf") as font:
            with patch.dict(os.environ, {"VIDEOGEN_FONT_FILE": font.name}):
                path, name = _find_preferred_font()
        self.assertEqual(font.name.replace("\\", "/"), path)
        self.assertEqual("Noto Sans CJK SC", name)
